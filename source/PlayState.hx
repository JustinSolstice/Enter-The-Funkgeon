...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		{
			if (Paths.fileExists('characters/' + storyChar + '-player.json', TEXT))
				storyChar = storyChar + '-player';
		...
		if (_fxShakeDuration > 0) // Window shake stuff I stole from FlxCamera
		{
			...
			else
			{
				if (_fxShakeAxes != FlxAxes.Y)
					Application.current.window.x = normalWindowX
						+ Math.round(FlxG.random.float(-_fxShakeIntensity * Application.current.window.width,
							_fxShakeIntensity * Application.current.window.width));
				if (_fxShakeAxes != FlxAxes.X)
					Application.current.window.y = normalWindowY
						+ Math.round(FlxG.random.float(-_fxShakeIntensity * Application.current.window.height,
							_fxShakeIntensity * Application.current.window.height));
			}
		}
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String)
	{
		switch (eventName)
		{
			...
			case 'Flash':
				...
				var val2:Float = Std.parseFloat(value2);
				if (FlxColor.fromString(val1) == null)
					val1 = 'WHITE';
	...
	public function endSong():Void
	{
		...
		else
		{
			var achieve:String = checkForAchievement([
				...
				'two_keys', 'toastie', 'debugger', 'reloadCursed'
			]);
		...
		if (Paths.formatToSongPath(SONG.song) != 'tutorial')
		{
			...
			if (health > (note.hitHealth * healthDrain))
			{
				if (!note.isSustainNote)
					...
					health -= (note.hitHealth * healthDrain) / 2;
			}
			else
				health = 0.01;
		}
	...
	var timeBarColorShitIDK:Bool = true;
	...
	override function beatHit()
	{
		...
		timeBarColorShitIDK = !timeBarColorShitIDK;
		if (timeBarColorShitIDK)
	...
	function shakeWindow(Intensity:Float = 0.05, Duration:Float = 0.5, Force:Bool = true, ?Axes:FlxAxes)
	{ // I'm just gonna use the code from FlxCamera and port it here
		if (Axes == null)
			Axes = XY;
