...
class PlayState extends MusicBeatState
{
	...
	public static var storyChar:String = '';
	...
	var normalWindowX = 320; //(Math.round(( Application.current.window.display.currentMode.width * 0.5))- Math.round((Application.current.window.width * 0.5))); Kinda pointless now since I can just set the vars in the event
	var normalWindowY = 180; //(Math.round(( Application.current.window.display.currentMode.height * 0.5))- Math.round((Application.current.window.height * 0.5)));
	...
	override public function create()
	{
		...
		switch (curStage) // add stage stuff here
		{
			case 'reloadStage': //Week 1
				{
				var bg:BGSprite = new BGSprite('reloadStage', -800, -550, 1, 0.9);
				add(bg);
				}
		...
		if (SONG.player1 == 'bf' && isStoryMode)
			{
			if(Paths.fileExists('characters/' + storyChar + '-player.json', TEXT)) storyChar = storyChar + '-player'; 
			boyfriend = new Boyfriend(0, 0, storyChar);
			}
		else
		...
		{
			timeBar.numDivisions = 400; //I'm gonna add this just in case
		}
		timeBar.alpha = 0;
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (ClientPrefs.getGameplaySetting('scrollspeed', 1) != 1) //Please don't make scroll speed a multiplier ShadowMario
	...
	override public function update(elapsed:Float)
	{
		...
		if (_fxShakeDuration > 0) //Window shake stuff I stole from FlxCamera
		{
			_fxShakeDuration -= elapsed;
			if (_fxShakeDuration <= 0)
			{
				Application.current.window.x = normalWindowX;
				Application.current.window.y = normalWindowY;
			}
			else
			{
				if (_fxShakeAxes != FlxAxes.Y) Application.current.window.x = normalWindowX + Math.round(FlxG.random.float(-_fxShakeIntensity * Application.current.window.width, _fxShakeIntensity * Application.current.window.width));
				if (_fxShakeAxes != FlxAxes.X) Application.current.window.y = normalWindowY + Math.round(FlxG.random.float(-_fxShakeIntensity * Application.current.window.height, _fxShakeIntensity * Application.current.window.height));
			}
		}
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Force Fullscreen':
				...
				if (value == 2) FlxG.fullscreen = true;
				else if (value == 1) FlxG.fullscreen = false;
				else FlxG.fullscreen = !FlxG.fullscreen;
			case 'Window Shake':
				normalWindowX = Application.current.window.x;
				normalWindowY = Application.current.window.y;
			...
			case 'Flash':
				...
				var val2:Float = Std.parseFloat(value2);
				if (FlxColor.fromString(val1) == null) val1 = 'WHITE';
				camHUD.flash(FlxColor.fromString(val1),val2);
		}
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['week1_nomiss', 'week2_nomiss', 'week3_nomiss', 'week4_nomiss',
				...
				'ur_good', 'hype', 'two_keys', 'toastie', 'debugger','reloadCursed']);
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		if(daNote.noteType == 'Bullet') {
			...
			FlxG.sound.play(Paths.sound('gun'), 0.8);
		}
		if(daNote.noteType == 'Jammed Bullets') {
			if(boyfriend.animation.getByName('hurt') != null) {
				boyfriend.playAnim('hurt', true);
				boyfriend.specialAnim = true;
			...
			camGame.shake(1, 0.2);
			FlxG.sound.play(Paths.sound('gun'), 1);
			doDeathCheck();
		}
	...
	function opponentNoteHit(note:Note):Void
	{
		if (Paths.formatToSongPath(SONG.song) != 'tutorial')
		{
			...
			if (health > (note.hitHealth * healthDrain))
			{
				if (!note.isSustainNote) health -= (note.hitHealth * healthDrain);
				else health -= (note.hitHealth * healthDrain) /2;
			}
			else health = 0.01;
		}
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if(note.noteType == 'Jammed Bullets') {
				if(boyfriend.animation.getByName('dodge') != null) {
					boyfriend.playAnim('dodge', true);
					boyfriend.specialAnim = true;
				}
	...
	function shakeWindow(Intensity:Float = 0.05, Duration:Float = 0.5, Force:Bool = true, ?Axes:FlxAxes) { //I'm just gonna use the code from FlxCamera and port it here
		if (Axes == null)
			Axes = XY;
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				switch(achievementName)
				{
					...
					case 'reloadCursed':
						if(CoolUtil.difficultyString() == 'CURSED' && !usedPractice && songName == 'reload') {
							unlock = true;
						}
					case 'ur_bad':
