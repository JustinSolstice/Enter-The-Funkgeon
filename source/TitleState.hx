...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		gungeonSpr = new FlxSprite(0, FlxG.height * 0.6).loadGraphic(Paths.image('enter-the-gungeon'));
		add(gungeonSpr);
		gungeonSpr.visible = false;
		gungeonSpr.setGraphicSize(Std.int(gungeonSpr.width * 0.55));
		gungeonSpr.updateHitbox();
		gungeonSpr.screenCenter(X);
		gungeonSpr.antialiasing = ClientPrefs.globalAntialiasing;
		gungeonSpr.alpha = 0;
	...
	function createCoolText(textArray:Array<String>, ?offset:Float = 0)
	{
		for (i in 0...textArray.length)
		{
			...
			textGroup.add(money);
			money.alpha = 0;
			FlxTween.tween(money,{alpha: 1}, 0.2, {ease: FlxEase.linear});
			}
		}
	}
	...
	function addMoreText(text:String, ?offset:Float = 0)
	{
		if(textGroup != null && credGroup != null) {
			...
			coolText.alpha = 0;
			FlxTween.tween(coolText,{alpha: 1}, 0.2, {ease: FlxEase.linear});
		}
	}
	...
	//Yea but now when I use this the music is offsync with the starting text -Solstice
	...
	override function beatHit()
	{
		...
		//	sickBeats++;
			switch (curBeat)
			{
				case 2: //case 1 dosent work for some reason
					createCoolText(['Neutron'], -20);
				case 3:
					addMoreText('fakeburritos', -20);
				case 4:
					addMoreText('Dool', -20);
				case 5:
					addMoreText('rocky', -20);
				case 6:
					addMoreText('Solstice', -20);
				case 8:
					addMoreText('Present...', 20);
				case 9:
					deleteCoolText();
				case 10:
					createCoolText(['This is a mod to'], -30);
				case 12:
					addMoreText('This game right below lol', -30);
					logoSpr.visible = true;
					FlxTween.tween(logoSpr,{alpha: 1}, 0.2, {ease: FlxEase.linear});
				// credTextShit.text += '\nNewgrounds';
				case 13:
					deleteCoolText();
					logoSpr.visible = false;
				// credTextShit.visible = false;
				case 14:
					createCoolText(['Which is also based on'], -30);
				case 16:
					addMoreText('This other game right below', -30);
					gungeonSpr.visible = true;
					FlxTween.tween(gungeonSpr,{alpha: 1}, 0.2, {ease: FlxEase.linear});
				// credTextShit.text += '\nNewgrounds';
				case 17:
					deleteCoolText();
					gungeonSpr.visible = false;
				...
				// credTextShit.screenCenter();
				case 27:
					Application.current.window.title = 'Enter';
					addMoreText('Enter');
				// credTextShit.visible = true;
				case 28:
					Application.current.window.title = 'Enter The';
					addMoreText('The');
				// credTextShit.text += '\nNight';
				case 29:
					Application.current.window.title = 'Enter The Funkgeon';
					addMoreText('Funkgeon'); // credTextShit.text += '\nFunkin';
				case 30:
					skipIntro();
			}
		}
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			Application.current.window.title = "Enter The Funkgeon | Friday Night Funkin': Psych Engine";
